...
def write_token_cache(payload: str, user_key: Optional[str] = None):
    ...
    # Define o ID do segredo: específico do usuário ou global [cite: 6]
    if user_key:
        secret_id = f"msal-cache-{user_key.replace('@', '-').replace('.', '-')}"
    else:
    ...
    secret_path = f"{parent}/secrets/{secret_id}" [cite: 6]

    try:
        sm_client().get_secret(request={"name": secret_path})
    except Exception:
        # Cria o segredo caso não exista [cite: 7]
        sm_client().create_secret(
            request={
                "parent": parent,
                "secret_id": secret_id,
                "secret": {"replication": {"automatic": {}}}, [cite: 7]
            }
        )
...
def build_cache(user_key: Optional[str] = None) -> msal.SerializableTokenCache:
    #Inicia o cache MSAL carregando dados do Secret Manager[cite: 10]."""
...
def save_cache(cache: msal.SerializableTokenCache, user_key: Optional[str] = None):
    #Persiste alterações no cache[cite: 10]."""
